...
```svelte title="src/routes/+page.svelte" {2}
<script lang="ts">
...
```svelte title="src/routes/TaskInput.svelte" {1-2}
<label for="task-input">Add Task: </label>
<input id="task-input" bind:value={taskName} on:keydown={onKeydown} />
```
...
```svelte title="src/routes/TaskInput.svelte" {1-4}
<script lang="ts">
...
```svelte title="src/routes/+page.svelte" {3,28}
<script lang="ts">
  import type { Task } from '../types';
...
```svelte title="src/routes/TaskInput.svelte" {4,9}
<script lang="ts">
  export let taskName: string;
  export let onKeydown: (e: KeyboardEvent) => void;
...
```svelte title="src/routes/+page.svelte" {28}
<script lang="ts">
  import type { Task } from '../types';
  import TaskInput from './TaskInput.svelte';
...
```svelte title="src/routes/AddTask.svelte" {5-7}
<script lang="ts">
  export let taskName: string;
  export let addTask: () => void;
...
```svelte title="src/routes/+page.svelte" {24}
<script lang="ts">
  import type { Task } from '../types';
  import AddTask from './AddTask.svelte';
...
```svelte title="src/routes/+page.svelte" {2, 22-24}
<script lang="ts">
...
```svelte title="src/routes/AddTask.svelte" {2,4,7,13}
<script lang="ts">
...
```svelte title="src/routes/+page.svelte" {8,26}
<script lang="ts">
  import { afterUpdate } from 'svelte';
  import type { Task } from '../types';
  import AddTask from './AddTask.svelte';
...
<Callout>
	...
	course has a [React](/courses/todo-app-react/overview) version where this
	approach can be super powerful and helps not use fancy tools like useCallback,
	useMemo, etc.
</Callout>
...
```svelte title="src/routes/AddTask.svelte" {6-12,15,21}
<script lang="ts">
  export let addTask: (taskName: string) => void;
...
```svelte title="src/routes/+page.svelte" {8-13}
<script lang="ts">
  import { afterUpdate } from 'svelte';
  import type { Task } from '../types';
  import AddTask from './AddTask.svelte';
...
```svelte title="src/routes/AddTask.svelte" {15-19}
<script lang="ts">
  export let addTask: (taskName: string) => void;
