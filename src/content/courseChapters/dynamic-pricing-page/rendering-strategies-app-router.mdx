...
```tsx title="src/app/app-router/ssr/page.tsx" {1-11}
...
```tsx {1, 4-18, 20-21} title="src/app/app-router/ssr/page.tsx"
...
```ansi frame="none" showLineNumbers=false {6, 18}
...
```tsx title="src/app/app-router/[country]/page.tsx" {1-29}
import PricingSection from '@/components/PricingSection';
...
```tsx title='src/middleware.ts' {4-7, 15-18}
...
```tsx title="src/app/app-router/csr/page.tsx" {1-38}
'use client';
...
```ansi frame="none" showLineNumbers=false {9, 21}
# SOME OUTPUT OMMITTED FOR BREVITY
