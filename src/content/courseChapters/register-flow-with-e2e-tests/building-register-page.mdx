...
```tsx title="src/app/(auth)/register/page.tsx" {1-46}
'use client';
...
```ts title="src/app/(auth)/register/_actions/register.ts" {1-50}
'use server';
...
```tsx title="src/app/(marketing)/_components/header.tsx" {16-18}
import Link from 'next/link';
