...
```svelte title="src/routes/TaskList.svelte" {1-11}
<script lang="ts">
...
```svelte title="src/routes/+page.svelte" {4,19}
<script lang="ts">
  import type { Task } from '../types';
...
```svelte title="src/routes/TaskListItem.svelte" {1-7}
<script lang="ts">
  import type { Task } from '../types';
...
```svelte title="src/routes/TaskList.svelte" {3,10}
<script lang="ts">
  import type { Task } from '../types';
...
```svelte title="src/routes/TaskListItem.svelte" {1}
<li><slot /></li>
```

Now, we can use the `TaskListItem` component like this.

```svelte title="src/routes/TaskList.svelte" {10}
<script lang="ts">
  import type { Task } from '../types';
  import TaskListItem from './TaskListItem.svelte';
...
```svelte title="src/routes/TaskList.svelte" {1-3}
<ul>
  <slot />
</ul>
```
...
```svelte title="src/routes/+page.svelte" {4-5,20-24}
<script lang="ts">
  import type { Task } from '../types';
  import AddTask from './AddTask.svelte';
  import TaskList from './TaskList.svelte';
...
```svelte title="src/routes/TaskListHeader.svelte" {1-5}
<script lang="ts">
...
```svelte title="src/routes/+page.svelte" {5,22}
<script lang="ts">
  import type { Task } from '../types';
  import AddTask from './AddTask.svelte';
  import TaskList from './TaskList.svelte';
...
```svelte title="src/routes/TaskList.svelte" {1-3, 6, 8, 10, 11, 13}
<div>
  <slot name="header" />
  <ul>
    <slot />
...
```svelte title="src/routes/+page.svelte" {22}
<script lang="ts">
  import type { Task } from '../types';
  import AddTask from './AddTask.svelte';
  import TaskList from './TaskList.svelte';
...
```svelte title="src/routes/TaskList.svelte" {1-13,17}
<script lang="ts">
